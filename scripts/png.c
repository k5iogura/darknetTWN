...
result_t write_png_file(const char *filename, image_t *img) {
  find_replace(filename,".png",".rle",filename);
  ...
  switch (img->color_type) {
    case COLOR_TYPE_INDEX:  // インデックスカラー
      ...
      for(y=0;y<img->height;y++){
          int edge=-1, changes=0;
          for(x=0;x<img->width;x++){
              unsigned char pix = img->map[y][x].i;
              if(pix == 255) pix = 0;  // background
              if(edge != pix || x == img->width-1){
                  changes++;
                  edge = pix;
                  if(x == img->width-1){
                      fprintf(fp, ",%d ",x);
                  }else if(changes == 1){
                      fprintf(fp, "%d %d",pix,x);
                  }else{
                      fprintf(fp, ",%d ",x);
                      fprintf(fp, "%d %d",pix,x+1);
                  }
              }
          }
          fprintf(fp, "\n");
